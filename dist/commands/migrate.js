...
import { logger, isShopifyTheme, writePackageJsonSync, move, isShopifyThemeSettingsFile, downloadFromUrl, beautifyJson, isShopifyThemeWhitelistDir } from '../utils.js';
import { join } from 'path';
import { existsSync, mkdirSync, readdirSync } from 'fs';
import figures from 'figures';
export default function (cli) {
  cli.command('migrate').description('Converts an existing theme to work with Slate Structure.').option('--npm', 'installs theme dependencies with npm instead of yarn').action(async (options = {}) => {
    ...
    try {
      ...
      if (!existsSync(configYml)) {
        await downloadFromUrl('https://raw.githubusercontent.com/Shopify/slate/0.x/packages/slate-theme/config-sample.yml', configYml);
