...
export default function (cli) {
  cli.command('generate [name]').description("Generates a new theme directory containing Slate's theme boilerplate.").option('--npm', 'install theme dependencies with npm instaed of npm').action(async (name, options = {}) => {
    let dirName = name;
    if (!dirName) {
    ...
    mkdirSync(root);
    return downloadFromUrl(s3Url, join(root, 'starter-theme.zip')).then(file => unzip(file, root)).then(() => {
